...
<div id="container">
	...
	<div id="content">
		...
		<input type="hidden" name="bno"class="chnno">
			<table class="type03">
				<caption>
					* 리스트 게시판<span style="font-size: 13px; margin-left: 20px; letter-spacing:0px;  font-weight: normal;"><span style="font-size: 14px; color: #337ab7;">게시글수 : ${total}개</span></span> 
					<div style="float: right; width: 400px; text-align: right">
						<select name="searchCondition" style="height: 30px; font-size: 12px">
							<option value="t"
								<c:out value="${search.searchCondition == 't'?'selected':''}"/>>제목
							</option>
						</select> 
						<input type="text" name="searchText" value="${search.searchText}" id="searchText">
		...
		<c:forEach var='i' begin='${page.start}' end='${page.end}'>
			...
			<a class="page-link" href="/board/list?page=${i}&searchCondition=${search.searchCondition}&searchText=${search.searchText}">${i}</a></li>
		</c:forEach>
		<c:if test="${page.next}">
