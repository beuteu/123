...
<script type="text/javascript">
	...
	function boardList() {
		...
		var pageMaker = [];
				...
				$.ajax({
					url : 'http://localhost/board/listAjax.do' + "?page="
							+ page,
					dataType : 'json',
					type : 'GET',
					success : function(data) {
						...
						for (var i = 0; i < list.length; i++) {
							...
							listBoard += "<table class='type03' style='border-top:none'><tr style='cursor: pointer;' class='viewnotice'  data-toggle='modal' data-target='.bd-example-modal-lg' data-bno='"+list[i].bno+"' data-title='"+list[i].title+"'  data-content='"+list[i].content+"'  data-userId='"+list[i].userId+"'  data-rgsde='"+list[i].rgsde+"' ><td class='TableCenter' style='width:140px; height:30px; font-size:14px'>"
									+ list[i].bno
									+ "</td><td class='TableLeft' style='width:*;' >"
									+ list[i].title
	...
	$(document).ready(function() {
		...
		$("#register").on("click", function() {
			...
			$.ajax({
				url : 'http://localhost/board/register3.do',
				type : 'POST',
				data : obj,
				success : function() {
					alert("글이 등록 되었습니다");
		...
		<div class="modal-content" style="padding: 20px">
			<table class="type03">
				...
				<tr>
					...
					<td class='TableLeft'><input name="title"
						style="width: 600px; height: 38px;" id="title"></td>
				</tr>
				<tr>
					<th class='TableLeft'>내용</th>
					<td class=' TableLeft'><textarea rows="10" cols="50"
							name="content" style="width: 100%;" id="content2"> </textarea></td>
				</tr>
			</table>
